"""Hollywood Farmers Market source entrypoint.

    uv run python -m pipeline.sources.hollywood_farmers_market --output /tmp/hfm.json
"""
...
from . import config
from .fetch import HollywoodFetchError, fetch_raw
from .normalize import normalize

log = logsetup.get_logger("pipeline.sources.hollywood_farmers_market")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="pipeline.sources.hollywood_farmers_market")
    ...
    try:
        raw, hours_text, _ = fetch_raw()
    except HollywoodFetchError as exc:
        log.error("fetch failed: %s", exc)
        return 1

    events, counters = normalize(raw, hours_text=hours_text, now=now)
