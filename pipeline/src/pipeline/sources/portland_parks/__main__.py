"""Portland Parks Summer Free For All entrypoint.

    uv run python -m pipeline.sources.portland_parks --output /tmp/parks.json

Seasonal: outside summer the schedule table may be absent, which yields zero events
...
from . import config
from .fetch import ParksFetchError, fetch_raw
from .normalize import normalize

log = logsetup.get_logger("pipeline.sources.portland_parks")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="pipeline.sources.portland_parks")
    ...
    try:
        raw = fetch_raw(fallback_year=now.year)
    except ParksFetchError as exc:
