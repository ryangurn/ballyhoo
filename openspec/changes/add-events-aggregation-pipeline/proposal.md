...
- **Explicitly defer** any archive history-compaction workflow. Tiered retention bounds the archive's working tree without one; reclaiming git history is separate future work with roughly six to eight months of runway, measured against the live `archive` branch.
...
- **Archive storage growth:** roughly 23 MB/day gzipped in git history (~8 GB/year), measured against the live `archive` branch at a merged feed of 1,900 events across five sources, 473 KB gzipped. Tiered pruning keeps the browsable working tree at a few hundred files regardless. Compaction is deferred; see design for the timeline.
