...
- **Explicitly defer** any archive history-compaction workflow. Tiered retention bounds the archive's working tree without one; reclaiming git history is separate future work with roughly eight to ten months of runway.
...
- **Archive storage growth:** roughly 20 MB/day gzipped in git history (~7 GB/year), even though tiered pruning keeps the browsable working tree at a few hundred files. Compaction is deferred; see design for the timeline.
