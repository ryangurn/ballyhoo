...
- **Explicitly defer** any archive history-compaction workflow. Tiered retention bounds the archive's working tree without one; reclaiming git history is separate future work with roughly fifteen to eighteen months of runway.
...
- **Archive storage growth:** roughly 11 MB/day gzipped in git history (~4 GB/year), based on measured Ticketmaster volume of ~723 events over a 365-day window. Tiered pruning keeps the browsable working tree at a few hundred files regardless. Compaction is deferred; see design for the timeline.
