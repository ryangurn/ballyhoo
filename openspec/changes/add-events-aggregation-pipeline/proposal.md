...
- **Non-goals for this change:** civic sources (portland.gov, Multnomah County Library, Oregon Metro), per-venue scrapers (OMSI, Holocene, Portland Mercado). Each becomes its own OpenSpec proposal that adds one workflow YAML file and one source module. Also excluded: the archive history-compaction workflow, deliberately deferred.
