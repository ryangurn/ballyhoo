...
- [ ] 4.2 Compute the content SHA-256 over the payload with `generated_at` removed and compare against the artifact's most recent recorded content hash; skip writing and report "unchanged" when identical. Hashing the whole payload is what made this check unreachable the first time, since that stamp is fresh every run. Skip only when the current UTC day already has a daily-tier entry, so an artifact that sits still does not leave holes in a tier that promises one entry per day, and treat a manifest entry with no recorded content hash as no match so the failure mode is a redundant snapshot rather than a silently skipped one
...
- [ ] 4.6 Update the monthly manifest at `<artifact>/daily/<YYYY>/<MM>/index.json` with capture time, event count, byte size, and both hashes kept distinct — `sha256` checksumming the stored bytes and `content_sha256` being what dedup compares, since conflating the two is precisely what made the skip unreachable; keep manifests uncompressed
- [ ] 4.7 Update `<artifact>/recent/index.json` listing currently retained recent-tier snapshots
- [ ] 4.8 Unit-test: within one day, content identical apart from a moved `generated_at` writes one recent snapshot rather than two; changed content writes two; the first run of a new day writes even when unchanged and marks the entry `unchanged`; daily entry reflects the latest write of the day
...
- [ ] 15.6 Establish the `archive` branch's actual growth rate by observation, and only then write a runway back into `design.md` and `proposal.md`. Take the size from `git count-objects -vH` on a fresh bare clone of `archive` at the start and end of the window, so the figure is the real transferred history rather than a working tree. Run the window for at least three weeks with dedup active: a week is too short to trust here, because one pipeline deploy that changes normalizer output rewrites every event of a source at once and a single quiet weekend pulls the average as hard in the other direction — the point is that no one day can dominate the result. Report the per-day spread alongside the mean, not the mean alone, and cross-check it against the manifests by counting snapshots actually written versus skipped as `unchanged`; the two should agree, and if they do not, the dedup is not doing what this task assumes. List every pipeline deploy that landed inside the window and say whether the outlier days line up with them, since that is exactly the confusion that invalidated the previous estimate. Do not extrapolate from run cadence — cadence is what produced both wrong figures
