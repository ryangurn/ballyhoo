...
Sizing this against measured volume rather than guesswork: Ticketmaster contributes roughly 723 events over a 365-day window, so a merged feed of Ticketmaster plus Calagator lands near 750 KB uncompressed and about 110 KB gzipped. At roughly 72 merged snapshots and 48 per-source snapshots per day, history grows around **11 MB/day, or about 4 GB/year**. GitHub's recommended repository ceiling is around 5 GB, putting compaction roughly **15 to 18 months** out.
...
- **The `archive` branch's git history grows at roughly 4 GB/year even with tiered pruning.** Pruning bounds the working tree, not history. → Mitigation: the tiered layout makes a future orphan-commit compaction trivial and safe. Timeline is roughly fifteen to eighteen months before GitHub's ~5 GB recommended repo ceiling becomes a concern, which is ample runway for a deliberately deferred action. Worth setting a calendar reminder rather than discovering it from a GitHub warning email.
...
- How much does Calagator actually contribute? All volume figures here are measured for Ticketmaster only. Calagator's event count is still unknown, and it determines the true merged feed size and therefore the archive growth rate. Measure it during Milestone A before trusting the sizing estimates.
- When exactly should we write the deferred archive-compaction workflow? The math says roughly fifteen to eighteen months of runway based on measured Ticketmaster volume, but adding sources raises the rate. Worth re-checking the `archive` branch size once all planned sources are live rather than trusting today's estimate.
...
Resolved since this document was first written: the staging-feed question became the `add-staging-feed` change; the Sources-tab health question became the `add-source-health-ui` change; the Ticketmaster segment and time-horizon questions were settled by measuring live Portland data (see Decisions).
