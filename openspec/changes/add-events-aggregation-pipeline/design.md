...
**Bound Ticketmaster volume by a 365-day time window, sliced by date only as far as the API's ceiling requires.**
...
At the time those measurements were taken a full year landed near 723, well under the API's 1,000-item deep-pagination ceiling (`size * page < 1000`), and one paginated query per run sufficed. **That stopped being true on 2026-09-10**, when volume crossed the 900 guard and the source stopped publishing rather than truncate. Re-measured on 2026-09-12 the same window holds 929, distributed far more steeply than the original table suggests: the first 90 days hold 713 of them and the last 95 hold 4.

**Slice by date, and only as deeply as the ceiling forces.**
The whole window is requested first, and any range whose `page.totalElements` exceeds the guard is cut in half and each half requested in turn, recursively, until every range being paged is one the API will actually serve. A quiet window therefore still costs a single query, and today's 929 costs one split — 870 and 59, in 7 requests rather than 5.

Date is the only axis that is safe to cut on. Every event has exactly one start date, so cutting at an instant partitions the result set instead of resampling it; measured, twelve 30-day slices summed to exactly the 929 the unsliced query reported and agreed at every intermediate horizon. Segment-slicing was the alternative and is rejected for the reason above: it is lossy by construction.

Fixed slice widths were also considered and rejected. Quarters would put 718 events in Q1 against a 1,000 ceiling, which is the same problem deferred by a few months, and any fixed width has to be re-tuned by hand each time Portland outgrows it — which is precisely the failure being fixed. Bisection needs no tuning: each range splits exactly when it outgrows the guard.

**The guard now bounds a slice, not the run, and still fails loudly when slicing runs out.**
A run may legitimately collect far more than 900 events; what may not exceed it is any single query, because silent truncation at the 1,000th item is the worst failure available here — the feed would look fine while quietly missing events. Bisection stops at one day, and a single day above the guard fails the run with an explicit error rather than publishing a feed that is quietly incomplete. Events collected across slices are deduplicated on `id`, since upstream treats both range endpoints as inclusive.
...
- **Ticketmaster volume grew past what one query can serve.** This one happened: volume reached the 900 guard on 2026-09-10 and the source refused to publish for two days rather than truncate silently, which is the guard working as intended. → Resolved by date-slicing: oversized ranges bisect until each is servable, so growth no longer requires re-tuning. What remains is the floor — a single day above the guard cannot be cut finer and fails the run — which Portland is nowhere near. The guard firing again is now a signal to revisit the window or radius, not the fetch strategy.
