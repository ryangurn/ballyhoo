...
Deleting a file from a branch removes it from the current tree but leaves it in the commit history forever. So tiered pruning keeps the archive *browsable* (a few hundred files instead of tens of thousands) and makes a future compaction cheap, but the `archive` branch's `.git` still grows at the full per-run rate.

Sizing this against the `archive` branch as it actually exists, rather than against arithmetic: the merged feed is now 1,900 events across five sources, 2.4 MB uncompressed and **473 KB gzipped**, and that gzipped figure is what one merged snapshot costs in history. Gzip defeats git's delta compression almost entirely — every snapshot on the branch whose content changed at all beyond its `generated_at` stamp cost between 99.9% and 100.5% of its blob size, because one changed byte early in the stream diverges everything after it. Only snapshots that are byte-identical apart from the timestamp collapse into a delta, and those cost about 60 bytes. Growth is therefore just the number of genuinely-changed snapshots per day times their size. Five hourly sources, a merge on each source completion, and the hourly safety cron write about 120 per-source and 144 merged snapshots a day; Ticketmaster's payload changes on every single fetch, which by itself guarantees one full 473 KB merged snapshot an hour, and the slower sources add a handful more. That measures out at roughly **23 MB/day, or about 8 GB/year**, bounded below by about 14 MB/day if Ticketmaster were the only thing that ever changed and above by about 79 MB/day if all five sources changed every hour. GitHub's recommended repository ceiling is around 5 GB, putting compaction roughly **six to eight months** out. This figure is measured rather than estimated, and it is predicated on a 473 KB gzipped merged feed: that snapshot is by far the dominant term, so if the feed grows materially past this size or more sources land, the runway shortens close to proportionally and the number above is stale.
...
Each artifact's most recent snapshot hash is tracked in its manifest. If the new content hashes identically, no snapshot is written. Measured against the live branch, this check has so far skipped nothing at all: the hash covers the entire published payload, and every payload carries a `generated_at` stamp that is fresh on every run, so it can never match. What rescues the intent is an accident of `dump_json` sorting keys — `generated_at` sorts after `events`, so a run whose only change is the timestamp perturbs just the tail of the gzip stream and git stores the whole snapshot as a delta of around 60 bytes. The saving the check was meant to deliver does arrive, but from git rather than from the check; the check itself would only start earning its place if the timestamp moved outside the hashed payload.
...
- **The `archive` branch's git history grows at roughly 8 GB/year even with tiered pruning.** Pruning bounds the working tree, not history. → Mitigation: the tiered layout makes a future orphan-commit compaction trivial and safe. Measured against the live branch, the timeline is roughly six to eight months before GitHub's ~5 GB recommended repo ceiling becomes a concern — still runway, but not the comfortable margin the original estimate implied, and short enough that the compaction wants a scheduled date rather than an open-ended deferral. Worth setting a calendar reminder rather than discovering it from a GitHub warning email.
...
- When exactly should we write the deferred archive-compaction workflow? Measuring the live `archive` branch puts the runway at roughly six to eight months against a 473 KB gzipped feed, down from the fifteen to eighteen months originally estimated, and every source added shortens it further. That is close enough that the question is no longer "when should we re-check" but "what date do we pick" — ideally before the next batch of sources lands, since the rate scales with the merged feed and the merged feed only grows.
...
Resolved since this document was first written: the staging-feed question became the `add-staging-feed` change; the Sources-tab health question became the `add-source-health-ui` change; the Ticketmaster segment and time-horizon questions were settled by measuring live Portland data (see Decisions); and the question of how much Calagator contributes was overtaken by events, since it lands 27 events beside Ticketmaster's 704 and DoPDX's 1,128, and the sizing above is now measured across all five live sources rather than extrapolated from Ticketmaster alone.
